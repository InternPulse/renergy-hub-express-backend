...
export const createWishList = async (req: Request, res: Response, next: NextFunction) => {
  try 
  {
    const newWishList = await wishlistService.createWishList({ ...req.body, userId: parseInt(req.user?.userID) });
...
export const getAllWishListsForUser = async (req: Request, res: Response, next: NextFunction) => {
  try 
  {
    const wishlists = await wishlistService.getAllWishListsForUser(parseInt(req.user?.userID));
